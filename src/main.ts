...
async function getToolVersion(toolPath: string): Promise<string> {
  try {
    const {stdout, stderr, exitCode} = await exec.getExecOutput(toolPath, ['--version'], {
    ...
    let version = stdout.trim().substring(stdout.indexOf(' ')).trim();
...
async function findHeim() {
  ...
  let pathVersion = await getToolVersion(toolPath);

  if(version) {
    if(version === 'current' || version === 'latest') {
      let httpClient = new hc.HttpClient('exec heim', [], {
        allowRetries: true,
        maxRetries: 3
      });
      const resp = await httpClient.getJson<IHeimVersions>(`https://cloud.heim.dev/heim/release?version=${version}`);
      ...
      let condition = `>=${foundVersion} <${foundVersion?.inc('major')}`;
      if(foundVersion) {
        if(foundVersion.raw != pathVersion) {
          let evalVersion = tc.evaluateVersions(allVersions, condition);

          if(semver.lt(evalVersion, pathVersion) &&
            !semver.satisfies(pathVersion, condition)) {
            core.warning(`Found newer version in path than the expected version, ${version} < ${pathVersion}`);
...
async function readInputs(): Promise<IHeimInputs> {
  ...
  let component = core.getInput('component', { required: false });
  ...
  let host = core.getInput('host', { required: false });
  if(host) {
    let hostJson: IHeimHost = JSON.parse(host);
    args.push('--host', hostJson.addr);
    args.push('--port', `${hostJson.port}`);
  }
  let envs = core.getInput('envs', { required: false });
  if(envs) {
    let obj: object = JSON.parse(envs);
    let envsMap: Map<string, string> = new Map(Object.entries(obj));
  ...
  let componentPath = core.getInput('path', { required: false });
  if(componentPath) {
...
export async function run(): Promise<void> {
  try {
    let heimInputs = await readInputs();
    ...
    options.failOnStdErr = core.getBooleanInput('failOnStdErr', { required: false });
