...
describe('heim-exec', () => {
  ...
  it('No version', async () => {
    ...
    expect(setFailedSpy).toHaveBeenLastCalledWith(
      'Unable to find heim installation'
    )
  })
  ...
  it('No CI token', async () => {
    ...
    expect(setFailedSpy).toHaveBeenLastCalledWith(
      'HEIM_CI_TOKEN or token is required for deployment'
    )
  })
  ...
  it('Component not in workspace', async () => {
    ...
    const whichPath = path.normalize('/usr/bin/heim/bin/heim')
    ...
    expect(warningSpy).toHaveBeenCalledWith(
      'ignoring "component" outside of a workspace'
    )
  })
